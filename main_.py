...
def TrackImages():
    ...
    for i in df["NAME"]:
        ...
        if str(i) =="nan":
            ...
            face_list.append(i)
            try:
                f_image = face_recognition.load_image_file(f"{path}/{str(i).split(' ')[-1]}.jpg")
    ...
    #####----------------------------
        ...
        if exists:
            dff = pd.read_csv("Attendance/Attendance_" + date + ".csv", on_bad_lines='skip')
        # elif name == "Unknown":
        #     continue
        else: 
            # ------------------------------------------------------------------------
           # print('name  ', name)
            #student_mail = 'jakhon37@gmail.com'
            # atn_info = f' Student ID: {attendance[0]}\n Name: {attendance[2]}\n Date: {attendance[4]}\n Time: {attendance[6]}'
            # m_mail, m_password, mail_to_send, attn_info = 'jakhon@gachon.ac.kr', '#a123123', student_mail, atn_info
